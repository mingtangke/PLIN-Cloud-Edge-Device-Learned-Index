...
void Server(int port, const std::string &data_path){

    keys = new _key_t[number];
    payloads = new _payload_t[number];

    const std::string &file_name = data_path;
    std::ifstream infile(file_name);
    ...
    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) == -1) {
        fprintf(stderr, "fail to bind port %d, %s\n", port, strerror(errno));
        exit(EXIT_FAILURE);
    }
...
// Usage: ./s [port] [data_path]
//   defaults: port=PORT_DEFAULT (9999), data_path=../Data.txt
int main(int argc, char **argv){
    int port = (argc > 1) ? std::stoi(argv[1]) : PORT_DEFAULT;
    std::string data_path = (argc > 2) ? argv[2] : "../Data.txt";
    std::cout << "Server config: port=" << port << " data=" << data_path << std::endl;
    Server(port, data_path);
    return 0;
}