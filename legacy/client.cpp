...
//         // std::lock_guard<std::mutex> lock(cout_mutex);
//         std::string filename = "train_result.txt";
//         std::ofstream outfile(filename, std::ios_base::app);
...
void test(int num_threads, const std::string &workload_path) {
    std::vector<std::thread> threads;
    threads.reserve(num_threads);
    for (int i = 0; i < num_threads; ++i) {
        threads.emplace_back(concurrency, workload_path);
    }
    for (auto& t : threads) t.join();
    std::cout << "All clients have finished." << std::endl;
}

// Usage: ./c [num_threads] [workload_csv]
//   defaults: num_threads=1, workload_csv=../data/workload_log.csv
int main(int argc, char **argv) {
    int num_threads = (argc > 1) ? std::stoi(argv[1]) : 1;
    std::string workload_path = (argc > 2) ? argv[2] : "../data/workload_log.csv";
    std::cout << "Client starting with " << num_threads << " threads, workload="
              << workload_path << ", server_port=" << PORT_DEFAULT << std::endl;
    test(num_threads, workload_path);
    return 0;
}
